...
class TransformerCNN1d(nn.Module):
    def __init__(self,attention_type, feature_num, e_layers, num_heads, num_classes,win_size,device, dropout = 0.1,d_model = 512):
        ...
        # self.linear = nn.Linear(d_model//4 * win_size, d_model // 4 ,bias=True)
        self.linear = nn.Linear(d_model//4 + d_model , d_model ,bias=True)
    ...
    def forward(self, x):
        ...
        # cls token
        # cls_tokens = self.cls_token.expand(out.shape[0], -1, -1)  # B 1 D
        # out = torch.cat((cls_tokens, out), dim=1)                 # B W+1 D

        encoder_out = self.encoder(out)         # B W D 
        ...
        out = self.norm1(out)
        out = self.act1(out)
        CNN1d_out = self.maxpool(out)

        out = torch.cat((encoder_out,CNN1d_out),dim=2) 
        out = self.linear(out)
