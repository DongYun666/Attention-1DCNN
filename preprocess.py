...
class FE:
    def __init__(self,file_path,limit=np.inf):
        ...
        self.nstat = ns.netStat(np.nan, maxHost, maxSess)  # 传入 nan maxHost maxSess

    def _get_tshark_path(self):
        if platform.system() == 'Windows':
            return 'D:\wareshark\Wireshark\\tshark.exe'
    ...
    def __prep__(self):
        ...
        if self.parse_type == "tsv":
            ...
            while decrement:
                ...
                decrement = False
                try:
                    csv.field_size_limit(maxInt) # 返回当前解析器允许的最大字段大小,如果制定了参数,参数将成为新的最大字段大小.
                except OverflowError:
                    maxInt = int(maxInt / 10)
                    decrement = True
            # 运行完成后 maxInt 922337203
            print("counting lines in file...")
            num_lines = sum(1 for line in open(self.path))  # 对应数据的行数
            print("There are " + str(num_lines-1) + " Packets.")
            self.limit = min(self.limit, num_lines-1) #450000 个包  不包含最后一个
    ...
    def get_next_vector(self):
        ...
        if self.parse_type == "tsv":
            ...
            if row[4] != '':  # IPv4
                ...
                dstIP = row[18]
                IPtype = 1
            srcproto = row[6] + row[    #UDP 或 TCP 端口：两个端口字符串的串联将导致 OR “[tcp|udp]”
...
if __name__ == "__main__":
    datapath = "H://数据集//TON-IOT-数据集//TON_IOT_pcap2"
    for filname in os.listdir(datapath):
        if filname.endswith(".pcap"):
            print("处理文件：", filname)
            fe = FE(datapath +"//"+ filname)
            fvs = []
            cnt = 0
            while(True):
                cnt += 1
            ...
            f = np.array(fvs)
            np.save("./processdata/TON_IOT/"+filname[:-5]+"_Test.npy", f)
            print("保存文件：", filname[:-5]+"_Test.npy")
    print("处理完成")
